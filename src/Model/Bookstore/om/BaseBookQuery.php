...
namespace Tworzenieweb\Model\Bookstore\om;
...
use \PropelPDO;
use Tworzenieweb\Model\Bookstore\Author;
use Tworzenieweb\Model\Bookstore\Book;
use Tworzenieweb\Model\Bookstore\BookPeer;
use Tworzenieweb\Model\Bookstore\BookQuery;
use Tworzenieweb\Model\Bookstore\Publisher;
...
abstract class BaseBookQuery extends ModelCriteria
{
    ...
    public function __construct($dbName = null, $modelName = null, $modelAlias = null)
    {
        ...
        if (null === $modelName) {
            $modelName = 'Tworzenieweb\\Model\\Bookstore\\Book';
        }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Tworzenieweb\Model\Bookstore\PublisherQuery A secondary query class using the current class as primary query
     */
    public function usePublisherQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinPublisher($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Publisher', '\Tworzenieweb\Model\Bookstore\PublisherQuery');
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Tworzenieweb\Model\Bookstore\AuthorQuery A secondary query class using the current class as primary query
     */
    public function useAuthorQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinAuthor($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Author', '\Tworzenieweb\Model\Bookstore\AuthorQuery');
    }
